...
__all__ = ["RewardHighVelocity", "RewardUnhealthyPose", "RewardScale"]
...
class RewardHighVelocity(gym.RewardWrapper):
    ...
    def __init__(self, env, **kwargs):
        ...
        assert any(
            e in self.env.unwrapped.spec.id.lower() for e in allowed_envs
        ), "Env {self.env.unwrapped.spec.id.lower()} not allowed for RewardWrapper"
    ...
    def step(self, action):
        ...
        if "cheetah" in self.env.unwrapped.spec.id.lower():
            return (observation, self.new_reward(reward, info), done, info)
        if "walker" in self.env.unwrapped.spec.id.lower():
            return (observation, self.new_reward(reward, info), done, info)
    ...
    def new_reward(self, reward, info):
        ...
        penal = info["risky_state"] * self.penal_v_distr.rvs() * self.penal
...
class RewardUnhealthyPose(gym.RewardWrapper):
    ...
    def __init__(self, env, **kwargs):
        ...
        elif "hopper" in self.env.unwrapped.spec.id.lower():
            self.robust_angle_range = (-0.1, 0.1)
            self.healthy_angle_range = (-0.2, 0.2)  # default env

        else:
            raise ValueError("Environment is not walker neither hopper " f"for {self.__class__.__name__}")
    ...
    def new_reward(self, reward):
        ...
        penal = (~self.is_robust_healthy) * (self.is_healthy) * self.penal_distr.rvs() * self.penal
