...
def evaluate(path: str, eval_episodes: int, capture_video: bool = False, seed=1, print_cvars: bool = False, normalize: bool = False):
    ...
    for _ in tqdm.tqdm(range(eval_episodes), desc="Evaluating Final Model", dynamic_ncols=True):
        ...
        while not done:
            actions = select_action(actor, actor_params, obs)
...
def create_expert_buffer(path, buffer_size, batch_size, seed=0):
    ...
    for _ in tqdm.tqdm(range(buffer_size), desc="Filling Expert Replay Buffer"):
        ...
        actions = np.array([jax.device_get(actions)[0]])
        actions = np.clip(actions, -1, 1)
        unscaled_actions = unscale_action(envs.single_action_space, actions)
        next_obs, rewards, dones, _ = envs.step(unscaled_actions)
        timestep = TimeStep(obs=obs[0], action=actions[0], reward=rewards[0], done=dones[0])
...
@partial(jax.jit, static_argnames="actor")
def select_action(actor: Actor, actor_params: flax.core.FrozenDict, observations: jnp.ndarray) -> jnp.ndarray:
    gaussian_action = actor.apply(actor_params, observations)[0]
    action = jnp.tanh(gaussian_action)
    return action
...
if __name__ == "__main__":
    ...
    key, actor_key, qf1_key, ent_key = jax.random.split(key, 4)
    ...
    def update_critic(
        ...
        qf1_next_target = qf1_next_target - ent_coef_value * next_log_prob.reshape(-1, 1)  # td error + entropy term
        next_q_value = rewards[..., None] + (1 - terminations[..., None]) * args.gamma * (qf1_next_target)
        ...
        return qf1_state, (qf1_loss_value, qf1_a_values), key
    ...
    def update_actor(
        ...
        def actor_loss(params):
            ...
            actor_loss = (ent_coef_value * log_prob[..., None] - qf1_a_values).mean()
            return actor_loss, -log_prob.mean()

        (actor_loss_value, entropy), grads = jax.value_and_grad(actor_loss, has_aux=True)(actor_state.params)
        actor_state = actor_state.apply_gradients(grads=grads)
        return actor_state, qf1_state, actor_loss_value, key, entropy
    ...
    for global_step in tqdm.tqdm(range(args.total_timesteps), smoothing=0.1, dynamic_ncols=True):
        ...
        if global_step > args.learning_starts:
            ...
            qf1_state, (qf1_loss_value, qf1_a_values), key = update_critic(
                actor_state,
                qf1_state,
                ent_coef_value,
            ...
            if global_step % args.policy_frequency == 0:
                (actor_state, qf1_state, actor_loss_value, key, entropy) = update_actor(
                    actor_state,
                    qf1_state,
                    ent_coef_value,
                    learn_batch.first.obs,
