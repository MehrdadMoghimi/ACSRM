...
def function_h(quantiles, s, c, theta_0, mu, config):
    # Calculate Q-values
    ...
    mu_spc_quantiles = jnp.multiply(mu_batch, jnp.minimum(0, diff))  # (batch_size, n_quantiles, n_quantiles)
...
def evaluate(path: str, eval_episodes: int, capture_video: bool = False, seed=1, print_cvars: bool = False, normalize: bool = False):
    ...
    for _ in tqdm.tqdm(range(eval_episodes), desc="Evaluating Final Model", dynamic_ncols=True):
        ...
        while not done:
            actions = actor.apply(actor_params, obs if args.extended_actor else obs[:, :-2])[0]
...
if __name__ == "__main__":
    ...
    initial_action, _ = actor.apply(actor_state.params, initial_state if args.extended_actor else initial_state[:, :-2])
    ...
    def update_critic(
        ...
        key, subkey = jax.random.split(key, 2)
        mean, log_std = actor.apply(actor_state.params, next_observations if args.extended_actor else next_observations[:, :-2])
    ...
    def update_actor(
        ...
        def actor_loss(params):
            ...
            mean_repeated, log_std_repeated = actor.apply(params, obs_repeated if args.extended_actor else obs_repeated[:, :, :-2])
            actions_repeated, _ = sample_action_and_log_prob(mean_repeated, log_std_repeated, v_subkey)  # (batch, n_sample, action_dim)
            vf1_a_values = qf.apply(qf1_state.params, obs_repeated if args.extended_critic else obs_repeated[:, :, :-2], actions_repeated).mean(1)
            vf2_a_values = qf.apply(qf2_state.params, obs_repeated if args.extended_critic else obs_repeated[:, :, :-2], actions_repeated).mean(1)
            v = jnp.minimum(vf1_a_values, vf2_a_values)
            v = function_h(quantiles=v, s=s, c=c, mu=mu, theta_0=theta_0, config=args)

            mean, log_std = actor.apply(params, observations if args.extended_actor else observations[:, :-2])
            ...
            qf2_a_values = qf.apply(qf2_state.params, observations if args.extended_critic else observations[:, :-2], actions)
            q = jnp.minimum(qf1_a_values, qf2_a_values)
            q = function_h(quantiles=q, s=s, c=c, mu=mu, theta_0=theta_0, config=args)
    ...
    for global_step in tqdm.tqdm(range(args.total_timesteps), smoothing=0.1, dynamic_ncols=True):
        ...
        if global_step < args.learning_starts:
            ...
            actions, key = sample_action(actor, actor_state, obs if args.extended_actor else obs[:, :-2], key)
