...
class AWACTrainer(NamedTuple):
    ...
    def update_actor(
        ...
        def actor_loss_fn(actor_params: flax.core.FrozenDict[str, Any]) -> jnp.ndarray:
            ...
            # pi_actions = dist.sample(sample_shape=args.n_samples, seed=rng).transpose(1, 0, 2)  # (batch_size, n_samples, action_dim)
            # obs_repeated = jnp.tile(batch.observations[..., None].transpose(0, 2, 1), (1, args.n_samples, 1))  # (batch_size, n_samples, obs_dim)
            # q_1, q_2 = agent.critic.apply_fn(agent.critic.params, obs_repeated, pi_actions)  # (batch_size, n_samples, n_quantiles)
            # q_1, q_2 = q_1.mean((1, 2)), q_2.mean((1, 2))
            pi_actions = dist.sample(seed=rng)
            q_1, q_2 = agent.critic.apply_fn(agent.critic.params, batch.observations, pi_actions)
            q_1, q_2 = jnp.matmul(q_1, phi_values) / args.n_quantiles, jnp.matmul(q_2, phi_values) / args.n_quantiles
            ...
            print(log_prob.shape, weights.shape, (log_prob * weights).shape)
            return loss
    ...
    def update_n_times(
        ...
        for _ in range(args.n_jitted_updates):  # we can jit for roop for static unroll
            ...
            if _ % args.policy_frequency == 0:
                agent, actor_loss = agent.update_actor(batch, actor_rng, args, phi_values)
