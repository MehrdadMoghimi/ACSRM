...
class AWACTrainer(NamedTuple):
    ...
    def update_actor(
        ...
        def actor_loss_fn(actor_params: flax.core.FrozenDict[str, Any]) -> jnp.ndarray:
            dist = agent.actor.apply_fn(actor_params, batch.observations if args.extended_actor else batch.observations[:, :-2])
            pi_actions = dist.sample(seed=rng)
            q_1, q_2 = agent.critic.apply_fn(
                agent.critic.params, batch.observations if args.extended_critic else batch.observations[:, :-2], pi_actions
            )
            s = args.reward_normalizer * batch.observations[:, -2] * (obs_std[-2] + 1e-5) + obs_mean[-2]
            c = batch.observations[:, -1] * (obs_std[-1] + 1e-5) + obs_mean[-1]
            q_1, q_2 = function_h(q_1, s, c, theta_0, mu, args), function_h(q_2, s, c, theta_0, mu, args)
            v = jnp.minimum(q_1, q_2)

            actions = jnp.clip(batch.actions, -agent.max_action, agent.max_action)
            q_1, q_2 = agent.critic.apply_fn(agent.critic.params, batch.observations if args.extended_critic else batch.observations[:, :-2], actions)
            q_1, q_2 = q_1, q_2 = function_h(q_1, s, c, theta_0, mu, args), function_h(q_2, s, c, theta_0, mu, args)
    ...
    def update_critic(
        ...
        def critic_loss_fn(critic_params: flax.core.FrozenDict[str, Any]) -> jnp.ndarray:
            ...
            target_next_action = agent.actor.apply_fn(
                agent.actor.params, batch.next_observations if args.extended_actor else batch.next_observations[:, :-2]
            ).sample(seed=rng)
    ...
    def update_h(agent, initial_state: np.ndarray, theta_0: jnp.ndarray):
        initial_action = agent.actor.apply_fn(agent.actor.params, initial_state if args.extended_actor else initial_state[:-2], temperature=0).sample(
...
def evaluate(path: str, eval_episodes: int, seed=1, print_cvars: bool = False, normalize: bool = False):
    ...
    for _ in tqdm.tqdm(range(eval_episodes), desc="Evaluating Final Model", dynamic_ncols=True):
        ...
        while not done:
            ...
            action = jax.device_get(policy_fn(actor_params, obs if args.extended_actor else obs[:-2]))
            action = unscale_action(env.action_space, action)
